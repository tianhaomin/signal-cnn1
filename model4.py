# -*- coding: utf-8 -*-
"""
Created on Wed Oct 18 09:36:26 2017
...
log_dir4 = 'D:/temp/tensorflow/project/logs/projectcnn_with_summaries4_1'
...
# 顺序打乱
label4 = label4[idx]
label4 = label4.astype(np.float32)
...
for i in range(10000):
    ...
    if i % 10 == 0:  # Record summaries and test-set accuracy
        summary, acc = sess4.run([merged, accuracy], feed_dict={x:test,y_:label4_test,keep_prob:1.0})
        ...
        if i % 100 == 99:  # Record execution stats
            ...
            summary, _ = sess4.run([merged, train_step],
                            feed_dict={x:train[a:a+10],y_:label4[a:a+10],keep_prob:0.8},
                            options=run_options,
                            run_metadata=run_metadata)
            train_writer.add_run_metadata(run_metadata, 'step%03d' % i)
            train_writer.add_summary(summary, i)
            saver.save(sess4, log_dir4+"/model.ckpt", i)
            print('Adding run metadata for', i)
        else:  # Record a summary
            summary, _ = sess4.run([merged, train_step], feed_dict={x:train[a:a+10],y_:label4[a:a+10],keep_prob:0.8})
            train_writer.add_summary(summary, i)
            #print(2)
train_writer.close()
test_writer.close()
